...
function Home({ token, history }: THome) {
    return (
        <div style={{ display: "flex", flex: 1, flexDirection: "column", padding: "20px" }}>
            <Heading data-test="Heading" title={"Medical Record Safe - Blockchain"} />
            <div data-test="contentContainer" className={classes.contentContainer}>
                <div data-test="divWithProjectWorkInfo" style={{ textAlign: "center" }}>
                    ...
                    <div className={classes.flex}>
                        <button data-test="button" className={classes.button} onClick={
                            () => {
                                if (token) {
                                    history.push("/blocks")
                                } else {
